...
def gen_batch(B,H,W,device,seed=None):
    ...
    noise=(torch.rand(B,1,1,1,generator=g)*0.015+0.005).to(device)
    eps=torch.randn(B,1,H,W,generator=g).to(device)
    b=Kf(imgs,otf)+noise*eps
...
def train(a):
    ...
    val_seed = a.val_seed if a.val_seed is not None else a.seed + 1000000
    vb,votf,vmu,_=gen_batch(a.batch,H,W,DEV,val_seed);best=1e9
    for it in range(1,a.iters+1):
        b,otf,mu,_=gen_batch(a.batch,H,W,DEV,a.seed*100000+it);prob=TV(b,otf,mu)
...
def evaluate(a):
    ...
    cfg=dict(tlo=0.1,thi=1.8,slo=0.02,shi=0.12,rho=0.9,tau0=0.3,s0=0.1,c=a.c)
    b,otf,mu,clean=gen_batch(a.ntest,H,W,DEV,a.test_seed);prob=TV(b,otf,mu)
    x0=b.clone();y0=torch.zeros_like(b).repeat(1,2,1,1)
    ...
    for k,xs in res.items():
        r=re(xs)
        metrics[k]={"err_K2":r[Ke//2],"err_K":r[-1]}
        for i,val in enumerate(r, start=1):
            curve_rows.append({"mode":a.mode,"method":k,"iteration":i,"median_rel_primal_error":val})
        print(f"{k:<26}{r[Ke//2]:>10.2e}{r[-1]:>10.2e}")
    if a.curves_csv:
        os.makedirs(os.path.dirname(a.curves_csv) or '.', exist_ok=True)
        with open(a.curves_csv,'w',newline='',encoding='utf-8') as f:
            w=csv.DictWriter(f,fieldnames=["mode","method","iteration","median_rel_primal_error"])
            w.writeheader(); w.writerows(curve_rows)
        print("saved",a.curves_csv)
    if a.metrics_json:
        os.makedirs(os.path.dirname(a.metrics_json) or '.', exist_ok=True)
        with open(a.metrics_json,'w',encoding='utf-8') as f:
            json.dump({"mode":a.mode,"seed":a.seed,"test_seed":a.test_seed,"K_eval":Ke,"metrics":metrics},f,indent=2)
        print("saved",a.metrics_json)
    try:
        ...
        plt.title(f'Track B warped ({a.mode})');plt.tight_layout();plt.savefig(a.plot_png,dpi=120)
        print("saved",a.plot_png)
...
if __name__=='__main__':
    ...
    ap.add_argument('--seed',type=int,default=1); ap.add_argument('--val_seed',type=int,default=None)
    ap.add_argument('--test_seed',type=int,default=2024)
    ...
    ap.add_argument('--curves_csv',default=None); ap.add_argument('--metrics_json',default=None)
    ap.add_argument('--plot_png',default='trackB_warped_eval.png')
